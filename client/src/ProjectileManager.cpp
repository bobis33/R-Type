...
namespace cli
{
    ecs::Entity ProjectileManager::createBasicProjectile(ecs::Registry &registry, float x, float y, float velocityX, float velocityY)
    {
        return createProjectile(registry, ecs::Projectile::BASIC, x, y, velocityX, velocityY);
    }

    ecs::Entity ProjectileManager::createSuperchargedProjectile(ecs::Registry &registry, float x, float y, float velocityX, float velocityY)
    {
        return createProjectile(registry, ecs::Projectile::SUPERCHARGED, x, y, velocityX, velocityY);
    }

    ecs::Entity ProjectileManager::createProjectile(ecs::Registry &registry, ecs::Projectile::Type type, float x, float y, float velocityX, float velocityY)
    {
        ...
        if (type == ecs::Projectile::BASIC)
        {
            return registry.createEntity()
                ...
                .with<ecs::Rect>("projectile_rect", 0.F, 0.F, static_cast<int>(Basic::SPRITE_WIDTH), static_cast<int>(Basic::SPRITE_HEIGHT))
        ...
        else
        {
            return registry.createEntity()
                .with<ecs::Transform>("projectile_transform", x, y, 0.F)
                .with<ecs::Velocity>("projectile_velocity", velocityX, velocityY)
                .with<ecs::Rect>("projectile_rect", 0.F, 0.F, static_cast<int>(Supercharged::SPRITE_WIDTH), static_cast<int>(Supercharged::SPRITE_HEIGHT))
                ...
                .with<ecs::Animation>("projectile_animation", 0, Supercharged::ANIMATION_FRAMES, 
                                    ...
                                    static_cast<int>(Supercharged::SPRITE_WIDTH), static_cast<int>(Supercharged::SPRITE_HEIGHT), 
                                    Supercharged::ANIMATION_FRAMES)
