...
void gme::HUDSystem::createScoreHUD(ecs::Registry &registry, float x, float y)
{
    m_scoreBgEntity = registry.createEntity()
                          ...
                          .with<ecs::Texture>("score_bg_texture", utl::Path::Texture::TEXTURE_SCORE_COUNTER_BG)
    ...
    for (int i = 0; i < 6; ++i)
    {
        auto digitEntity = registry.createEntity()
                               ...
                               .with<ecs::Texture>("score_digit_texture", utl::Path::Texture::TEXTURE_SCORE_DIGIT_0)
...
void gme::HUDSystem::updateScore(ecs::Registry &registry, int newScore) const
{
    ...
    static const char *digitPaths[10] = {
        utl::Path::Texture::TEXTURE_SCORE_DIGIT_0, utl::Path::Texture::TEXTURE_SCORE_DIGIT_1,
        utl::Path::Texture::TEXTURE_SCORE_DIGIT_2, utl::Path::Texture::TEXTURE_SCORE_DIGIT_3,
        utl::Path::Texture::TEXTURE_SCORE_DIGIT_4, utl::Path::Texture::TEXTURE_SCORE_DIGIT_5,
        utl::Path::Texture::TEXTURE_SCORE_DIGIT_6, utl::Path::Texture::TEXTURE_SCORE_DIGIT_7,
        utl::Path::Texture::TEXTURE_SCORE_DIGIT_8, utl::Path::Texture::TEXTURE_SCORE_DIGIT_9};
    ...
    for (int i = 0; i < 6 && i < m_scoreDigitEntities.size(); ++i)
    {
        ...
        if (auto *digitScale = registry.getComponent<ecs::Scale>(m_scoreDigitEntities[i]);
            (digitTransform != nullptr) && (digitScale != nullptr))
        {
