    ...
    void RTypeServer::updateEntities(float deltaTime)
    {
        ...
        for (const auto &[sessionId, entity] : players)
        {
            ...
            if (transform && velocity)
            {
                ...
                if (hitbox)
                {
                    ...
                    float maxX = GameConfig::Server::SCREEN_WIDTH - (PLAYER_WIDTH - PLAYER_HITBOX_RADIUS);
                    float minY = PLAYER_HITBOX_RADIUS;
                    float maxY = GameConfig::Server::SCREEN_HEIGHT - (PLAYER_HEIGHT - PLAYER_HITBOX_RADIUS);
                ...
                {
                    transform->x = std::max(0.0f, std::min(transform->x, GameConfig::Server::SCREEN_WIDTH));
                    transform->y = std::max(0.0f, std::min(transform->y, GameConfig::Server::SCREEN_HEIGHT));
                }
            }
        }
        ...
        for (const auto &[enemyId, entity] : enemies)
        {
            ...
            if (transform && velocity)
            {
                ...
                if (transform->x < -GameConfig::Server::WORLD_MARGIN ||
                    transform->x > GameConfig::Server::SCREEN_WIDTH + GameConfig::Server::WORLD_MARGIN ||
                    transform->y < -GameConfig::Server::WORLD_MARGIN ||
                    transform->y > GameConfig::Server::SCREEN_HEIGHT + GameConfig::Server::WORLD_MARGIN)
                {
        ...
        for (const auto &[projectileId, entity] : projectiles)
        {
            ...
            if (transform && velocity)
            {
                ...
                if (transform->x > GameConfig::Server::SCREEN_WIDTH + GameConfig::Server::WORLD_MARGIN ||
                    transform->x < -GameConfig::Server::WORLD_MARGIN ||
                    transform->y > GameConfig::Server::SCREEN_HEIGHT + GameConfig::Server::WORLD_MARGIN ||
                    transform->y < -GameConfig::Server::WORLD_MARGIN)
                {
