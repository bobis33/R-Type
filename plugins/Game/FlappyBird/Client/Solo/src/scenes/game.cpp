...
PipeType randomPipeType() {
    const int r = rand() % 3;
    if (r == 0) return PipeType::Small;
    if (r == 1) return PipeType::Medium;
    return PipeType::Large;
}

float getGapSize(const PipeType type) {
    switch (type) {
        case PipeType::Small: return 140.f;
        case PipeType::Medium: return 180.f;
        case PipeType::Large: return 220.f;
    }
    return 180.f;
}
    ...
    : AScene(assignedId), m_renderer(renderer), m_audio(audio), m_showDebug(showDebug)
{
    ...
    registry.onComponentAdded(
        [&renderer, &audio, &registry](const ecs::Entity e, const std::type_info &type)
        {
            ...
            if (hitBox && transform)
            {
                renderer->createCircleShape({
                    .name = "hitbox_" + std::to_string(e),
                ...
                if (transform && textureComp)
                {
                    if (rectComp)
                    {
                        renderer->createSprite(textureComp->id + std::to_string(e), textureComp->id,
                                               transform->x, transform->y, scale_x, scale_y,
    ...
    m_looseText = registry.createEntity()
    ...
    m_looseSound = registry.createEntity()
        .with<ecs::Audio>("audio" + std::to_string(m_looseSound), "assets/audio/flappy-die.ogg", 10.0F, false, false)
        .build();
    m_flapSound = registry.createEntity()
        .with<ecs::Audio>("audio" + std::to_string(m_flapSound), "assets/audio/flappy-flap.ogg", 10.0F, false, false)
        .build();
}
...
void gme::Game::update(const float dt, const eng::WindowSize &size)
{
    ...
    if (transform && velocity)
    {
        ...
        m_renderer->setSpritePosition("player_texture" + std::to_string(m_playerEntity),
                                      transform->x, transform->y);
    }
    ...
    for (auto &[topPipe, bottomPipe] : m_pipes)
    {
            ...
            topTransform && botTransform && topVel && botVel)
        {
            ...
            m_renderer->setSpritePosition("pipe_top_texture" + std::to_string(topPipe),
                                          topTransform->x, topTransform->y);
            m_renderer->setSpritePosition("pipe_bottom_texture" + std::to_string(bottomPipe),
                                          botTransform->x, botTransform->y);
        }
    }
...
{
    auto [offsetX, offsetY] = utl::calculateHitboxOffsets(33.0F, 17.0F, 2.0F);
