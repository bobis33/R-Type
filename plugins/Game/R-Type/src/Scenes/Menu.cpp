...
gme::Menu::Menu(const std::shared_ptr<eng::IRenderer> &renderer, const std::shared_ptr<eng::IAudio> &audio)
    : m_audio(audio)
{
    ...
    for (size_t i = 0; i < m_menuOptions.size(); ++i)
    {
        registry.createEntity()
            ...
            .with<ecs::Color>("color_menu", gme::Color::WHITE.r, gme::Color::WHITE.g, gme::Color::WHITE.b, gme::Color::WHITE.a)
...
void gme::Menu::update(const float dt, const eng::WindowSize &size)
{
...
void gme::Menu::event(const eng::Event &event)
{
    switch (event.type)
    {
        case eng::EventType::KeyPressed:
            ...
            {
                utl::Logger::log("Enter key detected! Selected index: " + std::to_string(m_selectedIndex), utl::LogLevel::INFO);
                const std::string &selectedOption =
                    m_menuOptions[static_cast<int>(m_menuOptions.size()) - 1 - m_selectedIndex];
                utl::Logger::log("Selected option: " + selectedOption, utl::LogLevel::INFO);
                if (onOptionSelected)
                {
                    utl::Logger::log("Calling onOptionSelected callback", utl::LogLevel::INFO);
                    onOptionSelected(selectedOption);
                }
                else
                {
                    utl::Logger::log("WARNING: onOptionSelected callback is NULL!", utl::LogLevel::WARNING);
                }
            }
            break;
